...
## 2026-07-16 第四輪:D 類架構債完成——Particle/Fx 改用 Canvas + Lighting 改用 DOM ref + 鏡頭/震動 transform 脫離 React

> 接續第二輪「PlayScene/BossScene 判定場/彈幕場改用 Canvas」那個切片
> 明確列出的範圍邊界(`ParticleLayer.jsx`/`LightingLayer.jsx`/
> `effect/FxLayer.jsx`/鏡頭震動 transform 都刻意留到之後)——使用者問
> 「還有什麼沒作」,盤點後確認這幾項就是剩下的 D 類架構債,使用者接著
> 明確指示「把 D 類作完,我一次測」,這一輪把清單全部做完。

**`ParticleLayer.jsx` 改用 Canvas 渲染**:跟第二輪 `PlayScene`/
`BossScene` 判定場/彈幕場同一個理由——原本每顆存活粒子是一個獨立
absolute-positioned div,combo 里程碑/爆炸這類噴發常態一次多達幾十顆
同時存活,且不像音符/彈幕有數量上限,是這個專案 DOM 節點 churn 最嚴重
的一層。改成單一 `<canvas>` 每幀用 `ctx.arc()+shadowBlur` 畫,半徑/
顏色/透明度/旋轉/縮放/發光陰影逐一還原原本 CSS 版本的視覺,`particleManager.js`
本身的位移物理模擬完全不變,只換渲染手法。

**`effect/FxLayer.jsx` 改用 Canvas 渲染**:同樣理由,判定飄字
(PERFECT/GREAT/GOOD/MISS)跟爆炸/spark 等特效原本每個是一個獨立文字
div,改成 `ctx.fillText()+shadowBlur` 畫。⚠️ canvas 文字渲染 emoji 的
字形品質視瀏覽器/作業系統內建字型而定,跟原生 emoji 渲染可能有些微
差異,是 canvas 文字渲染的已知限制,不是這次接線的邏輯錯誤,可接受的
近似。

**`LightingLayer.jsx`——刻意不用 Canvas,改用 DOM ref 直接寫**:跟上面
兩個不一樣,這層不是「每幀節點數會累積」的問題——查證過 `lighting.js`
的 `LIGHTING_PRESETS`,同時存活的頻道數固定是 3 個
(`bossPhaseAlert`/`dangerVignette`/`comboAura`),不會無限增加,沒有
DOM churn 要解決,換 Canvas 反而是過度工程。真正的浪費是原本每幀都呼叫
`setTick()` 強制整個元件重新 render(即使 90% 以上的幀裡這幾個頻道的
intensity 根本沒變)。改成掛載時就把這 3 個已知頻道的 overlay div 一次
建好(`CHANNEL_NAMES` 寫死陣列,之後 React 不會再增減這批節點),rAF
迴圈裡直接寫 `el.style.opacity/background/mixBlendMode`,完全不呼叫
`setState`——這個元件掛載後就不會再觸發任何一次 React re-render。
⚠️ 範圍邊界:`CHANNEL_NAMES` 是寫死列舉,不是動態讀 `getAll()` 的
key,之後 `lighting.js` 新增頻道名稱時要記得同步更新這個陣列,否則新
頻道會被觸發但畫不出來(不會報錯,只是視覺上沒反應)。

**`PlayScene.jsx`/`BossScene.jsx` 鏡頭/震動 transform 脫離 React**:
兩個場景原本每幀把 `cameraStyle`/`shakeStyle` 這兩個含 `transform` 的
物件存進 `viewRef.current`,再整包 spread 進 JSX 的 `style={{...}}`——
即使已經收斂成單一 `viewRef` + 一顆 `renderTick` state(第一輪效能
優化的成果),每次 `bumpRender()` 觸發的 re-render 仍然會讓 React 重新
比對這兩個 style 物件。這輪新增 `cameraWrapRef`(包住原本 `camera 縮放/
平移`那層 div),`tick()` 內直接 `cameraWrapRef.current.style.transform =
...`/`fieldBoxRef.current.style.transform = ...`,並把 `transform`
整個從 JSX style 物件裡拿掉。關鍵原理:React 的 style-prop diffing 只
管理「這次 render 的 JSX style 物件裡有出現的 CSS 屬性 key」,`transform`
既然已經不在 JSX style 物件裡,React 完全不會去動它,ref 的直接寫入
可以安全地跨越後續所有 React re-render 持續生效,不會被覆蓋或衝突。

- ⚠️ **範圍邊界**:HUD 面板/攻擊特效疊圖/QTE/平衡對抗閘門疊層這些低頻/
  瞬時的 UI 疊層,跟公事包 QTE 進度條這類「事件觸發才變」的顯示,都
  刻意維持原本 React state 寫法不動,理由跟第二輪同一套判準——只換
  「每幀持續變動、成本最高」的那幾層,不是全部重寫。
- 至此,前一輪 README 明訂的 D 類架構債清單(`ParticleLayer.jsx`/
  `LightingLayer.jsx`/`effect/FxLayer.jsx`/鏡頭震動 transform)已經全部
  處理完畢。仍然不是規格書講的完整「game loop 完全脫離 React」重構
  (那需要整個場景換成 Canvas/WebGL,連 HUD/UI 疊層都不例外),但這個
  專案目前每幀持續變動、DOM 節點會隨遊玩時間/連段/彈幕密度累積的幾個
  熱點都已經改完。

**驗證**:沙箱 `npm run build`(107 modules)通過;既有全部 14 支測試
全數回歸通過,零失敗(`test-b1-keybind-verify`/`test-b3-balance-verify`/
`test-boss`/`test-camera`/`test-evalrun`/`test-items-rogue`/
`test-judge-parity`/`test-miss-wiring`/`test-npc`/`test-particle`/
`test-preorder-verify`/`test-scene`/`test-songselect`/`test-ui`)——這輪
改動同樣全部是渲染層,沒有動到任何判定/資料邏輯,預期跟結果一致。
**完全沒有經過瀏覽器實測**,尤其鏡頭/震動 transform 改用 DOM ref 這塊
建議優先手動確認:BOSS 戰階段警示暗角/combo 光暈疊層顏色是否正確、
鏡頭縮放平移/震動視覺是否跟改動前一致(理論上像素級相同,因為只是把
同一個 transform 字串從「React 算好塞進 style prop」改成「直接寫
`el.style.transform`」,但仍需實機肉眼確認)。
